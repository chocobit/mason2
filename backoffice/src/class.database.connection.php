<?php
  ...
  class DB
  {
    ...
    function DB( $base, $server, $user, $pass)
    {
      ...
      //mysql_connect($server, $user, $pass) or die('Server connection not possible.-->conn2');
      //mysql_select_db($base)               or die('Database connex2ion not possible.-->db');
      $link = mysqli_connect($server, $user, $pass ,$base) or die('Server connection not possible.-->conn2');
      mysqli_connect($server, $user, $pass ,$base) or die('Server connection not possible.-->conn2');
      if (!$link) {
      ...
      //$dsn = 'mysql:dbname=toa-chugoku_db;host=evaii';
      //$user = 'root';
      //$password = 'Web$903054';
      /*
      try {
    ...
    function fetchNextObject($result = NULL)
    {
      ...
      if ($result == NULL || num_rows($result) < 1)
        return NULL;
      else
